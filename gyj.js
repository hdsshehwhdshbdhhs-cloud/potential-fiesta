...
const BOT_TOKEN = "7644231051:AAFswLeZkQBLIp40e1BdCNuc_XAxK_4owJY"; // Insert your bot token.
const BOT_WEBHOOK = "/endpoint"; // Let it be as it is.
const BOT_SECRET = "BOT_SECRET"; // Insert a powerful secret text (only [A-Z, a-z, 0-9, _, -] are allowed).
const BOT_OWNER = 6490007953; // Insert your telegram account id.
const BOT_CHANNEL = -1002438744271; // Insert your telegram channel id which the bot is admin in.
const SIA_NUMBER = 123445; // Insert a random integer number and keep it safe.
const PUBLIC_BOT = true; // true = sab use kar sakte, false = sirf BOT_OWNER

// Channel that users MUST join to use the bot:
const REQUIRED_CHANNEL = "@kajal_developer"; // e.g. "@HanimeDekho"

// Welcome text ( %name% ko user ke first name se replace kiya jaayega )
const WELCOME_TEXT =
  "HI THERE... %name%! 💥\n\n" +
  "I AM A FILE STORE BOT...!\n" +
  "I CAN PROVIDE PRIVATE FILES THROUGH A SPECIFIC LINK....!";
...
async function isUserInRequiredChannel(userId) {
  try {
...
function buildWelcomeText(name) {
  return WELCOME_TEXT.replace("%name%", name || "Friend");
}
...
async function onMessage(event, message) {
  ...
  // ---------- /start (welcome message like screenshot) ---------- //
  if (message.text && message.text === "/start") {
    const firstName = message.from.first_name || "Friend";
    const channelUsername = REQUIRED_CHANNEL.startsWith("@")
      ? REQUIRED_CHANNEL.slice(1)
      : REQUIRED_CHANNEL;
    ...
    const welcome = buildWelcomeText(firstName);
    return sendMessage(message.chat.id, message.message_id, welcome, buttons);
  }

  // ---------- Channel Join Verification (for everything except /start) ---------- //
  if (PUBLIC_BOT) {
    const userId = message.from.id;
    const inChannel = await isUserInRequiredChannel(userId);
    ...
    if (!inChannel) {
      ...
      return sendMessage(
        message.chat.id,
        message.message_id,
        "*⚠️ Verification Required*\n\nPlease join our channel first, then send /start or any file again.",
        buttons
      );
    }
  }

  // ---------- Private bot restriction ---------- //
